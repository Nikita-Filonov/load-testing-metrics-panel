...
export type LoadTestResultsContextProps = {
  ...
  getLoadTestResultDetails: (loadTestResultId: number) => Promise<void>;
};
...
const LoadTestResultsProvider: FC<PropsWithChildren> = ({ children }) => {
  ...
  const getLoadTestResultDetailsAPI = async (loadTestResultId: number) => {
    setLoading({ ...loading, getLoadTestResultDetails: true });
    const response = await loadTestResultsHTTPClient.getLoadTestResultDetails(loadTestResultId);
